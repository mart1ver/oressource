...
// Insertion de la collecte (sans les pesées) l'aide d'une requête préparée
  ...
  $req->execute(array($antidate,$_POST['id_filiere'], "sortiesr", $_POST['id_point_sortie'], $_POST['commentaire'], $_SESSION['id']));
...
$req->execute(array($antidate,$_POST["m".$i],  $id_sortie , $i, $_SESSION['id']));
  $req->closeCursor();
}
    $i++;
}
...
// Insertion de la collecte (sans les pesées) l'aide d'une requête préparée
	$req = $bdd->prepare('INSERT INTO sorties (id_filiere , classe, id_point_sortie, commentaire, id_createur) VALUES(?,?, ?, ?, ?)');
	$req->execute(array($_POST['id_filiere'], "sortiesr", $_POST['id_point_sortie'], $_POST['commentaire'], $_SESSION['id']));
  $id_sortie = $bdd->lastInsertId();
    $req->closeCursor();
...
$req->execute(array($_POST["m".$i],  $id_sortie , $i, $_SESSION['id']));
  $req->closeCursor();
}
    $i++;
}
